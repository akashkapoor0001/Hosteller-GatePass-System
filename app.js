...
app.post('/status', function(request, response) {
    var un = request.session.username; // Retrieve the username from the session
    console.log('status', un); // Log the username for debugging purposes

    // Query the database to get the gate pass status for the current user
	connection.query('SELECT * FROM gatepass where roll=?', [un], function(err, rows) {
		if (err) {
			// If there is an error, flash an error message and render the status page with no data
			request.flash('error', err); 
			response.render(__dirname + "/status.ejs", { page_title: "Users - Node.js", data: '' });   
		} else {
			// If the query is successful, render the status page with the retrieved data
			console.log('Gatepass Data: ', rows); // Log the retrieved data for debugging purposes
